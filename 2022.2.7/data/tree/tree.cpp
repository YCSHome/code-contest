...
#define M ((L + R) >> 1)
  ...
  void build(int o, int L, int R) {
    if (L == R)
      ...
      build(lc, L, M), build(rc, M + 1, R), sum[o] = sum[lc] + sum[rc];
  }

  void add(int o, int L, int R, int l, int r, LL w) {
    if (l <= L && R <= r)
      addv[o] += w, sum[o] += (R - L + 1) * w;
    else {
...
int getch(int u, int f) { return find(u, dep[u] - dep[f] - 1); }
